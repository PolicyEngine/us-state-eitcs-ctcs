...
def calculate_district_metrics(hh_data, person_data, districts):
    ...
    for district in districts:
        ...
        net_income = (hh_subset['household_net_income'] * hh_subset['household_weight']).sum()
        ...
        gini_weights = (hh_subset['household_weight'] * hh_subset['household_count_people']).values
        ...
        if total_person_weight > 0:
            poverty = (person_subset['in_poverty'] * person_subset['person_weight']).sum() / total_person_weight
        else:
            poverty = 0
        ...
        if total_child_weight > 0:
            child_poverty = (children['in_poverty'] * children['person_weight']).sum() / total_child_weight
        else:
            child_poverty = 0
...
def process_state(state, year=YEAR):
    ...
    for reform_name, reform_vars in [
        ...
        reform_results[reform_name] = calculate_district_metrics(hh_reform, person_reform, districts)
    ...
    for district in baseline_metrics:
        ...
        for reform_type, reform_metrics in reform_results.items():
            ...
            child_poverty_pct_cut = -safe_pct_change(baseline['child_poverty'], reform['child_poverty'])
            poverty_gap_pct_cut = -safe_pct_change(baseline['poverty_gap'], reform['poverty_gap'])
            gini_index_pct_cut = -safe_pct_change(baseline['gini_index'], reform['gini_index'])
...
def generate_all_data():
    ...
    district_df = district_df.sort_values(['state_fips', 'congressional_district_geoid', 'reform_type'])
